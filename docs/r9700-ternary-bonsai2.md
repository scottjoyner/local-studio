...
- A deterministic `summary.candidatePromotable` result. Promotion requires exact GPU architecture + R9700 identity, a clean exact Local Studio Git revision, hashed model/runtime provenance, a successful completion probe, the expected native structured tool call, at least one hashed file-backed OpenCode or Hermes session, and either a successful live controller benchmark or a hashed benchmark artifact.
